...
@app.post("/api/library/{library_id}/mix_ready_export")
def mix_ready_export(
    library_id: str,
    payload: dict = Body(
        default={"target_minutes": 90, "sort_by": "bpm"},
        description="Mix-ready export parameters",
    ),
):
    """Return a plain-text, mix-ready track list for quick set planning.

    - Optionally trims by target_minutes (rough 6 min/track fallback).
    - Sorts by BPM / year / key depending on sort_by.
    """
    lib = get_library_or_404(library_id)

    tracks = list(lib.tracks)
    sort_by = str(payload.get("sort_by", "bpm") or "bpm").lower()
    target_minutes = payload.get("target_minutes")

    if sort_by == "year":
        tracks.sort(key=lambda t: (getattr(t, "year", None) is None, getattr(t, "year", 0)))
    elif sort_by == "key":
        tracks.sort(key=lambda t: (getattr(t, "key", None) is None, getattr(t, "key", "")))
    else:
        tracks.sort(key=lambda t: (getattr(t, "bpm", None) is None, getattr(t, "bpm", 0)))

    if isinstance(target_minutes, int) and target_minutes > 0:
        approx_tracks = max(1, target_minutes // 6)
        tracks = tracks[:approx_tracks]

    lines_out = ["# BeatPorter mix‑ready export", f"# Library: {library_id}", ""]
    for idx, t in enumerate(tracks, start=1):
        ...
        main = " - ".join(parts) if parts else (t.file_path or "(untitled)")
        bpm_str = f"{t.bpm:.1f}" if getattr(t, "bpm", None) is not None else "?"
        key_str = (t.key or "?")
        year_val = getattr(t, "year", None)
        year_str = str(year_val) if year_val is not None else "?"
        lines_out.append(f"{idx:02d}. {main}  [BPM {bpm_str} | Key {key_str} | Year {year_str}]")
